...
function printInstallHelp(): void {
  console.log(`hyperfix-mcp install — register HyperFix in an MCP client config

Usage:
  hyperfix-mcp install [options]
...
Options:
  ...
  --name <string>     MCP server key under mcpServers (default: hyperfix)
...
Examples:
  ...
  hyperfix-mcp install

  hyperfix-mcp install --target cursor-user -y
  hyperfix-mcp install --target custom --output /path/to/mcp.json -y
`);
}
