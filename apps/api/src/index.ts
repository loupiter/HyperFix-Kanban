...
export function createApp() {
  ...
  const honoOpenApiHandler = openAPIRouteHandler(api, {
    documentation: {
      ...
      info: {
        ...
        version: "1.0.0",
        description:
          "HyperFix Project Management API - Manage projects, tasks, labels, and more",
      },
      servers: [
        {
          url: normalizeApiServerUrl(
            process.env.KANEO_API_URL || "https://coubeche.hypeer.cloud",
          ),
          description: "HyperFix API Server",
        },
      ],
      components: {
        securitySchemes: {
  ...
  api.use("*", async (c, next) => {
    ...
    const windowId = c.req.header("X-HyperFix-Window-Id");
