...
function getAuthEmailCopy(locale?: string | null) {
  return getLocaleKey(locale) === "de"
    ? {
        magicLinkSubject: "Anmeldelink fuer HyperFix",
        otpSubject: "Bestaetigungscode fuer HyperFix",
      }
    : {
        magicLinkSubject: "Login for HyperFix",
        otpSubject: "Authentication code for HyperFix",
      };
}
...
function getDeviceAuthClientIds(): Set<string> {
  ...
  return new Set(["hyperfix-cli", "hyperfix-mcp"]);
}
...
function getInvitationEmailSubject(
  ...
  return getLocaleKey(locale) === "de"
    ? `${inviterName} hat dich eingeladen, ${workspaceName} auf HyperFix beizutreten`
    : `${inviterName} invited you to join ${workspaceName} on HyperFix`;
}
    ...
    ...(process.env.DISABLE_GUEST_ACCESS !== "true"
      ? [
          anonymous({
            ...
            emailDomainName: "coubeche.hypeer.cloud",
          }),
        ]
      : []),
    lastLoginMethod(),
