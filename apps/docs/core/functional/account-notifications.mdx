...
description: Deliver your personal HyperFix notifications by email, ntfy, Gotify, or a custom webhook with workspace and project scoping
---

HyperFix can keep the normal in-app notification inbox and also forward matching notifications to external channels for the signed-in user.
...
HyperFix already stores in-app notifications in the notification inbox. Account notification delivery adds a separate preference layer on top of that behavior.
...
- create an application in your Gotify server before connecting HyperFix
- paste the base server URL, for example `https://gotify.example.com`
- no extra topic or path is required because HyperFix sends directly to the Gotify message API using the application token
...
When a signing secret is configured, HyperFix sends an `X-HyperFix-Signature` header containing the `HMAC-SHA256` hex digest of the raw JSON body.

## Workspace delivery rules

Below the global channel cards, HyperFix shows workspace delivery rules.
...
V1 forwards the same notification types that currently create inbox notifications in HyperFix.
...
If HyperFix cannot resolve a notification to a workspace or project context, that notification stays inbox-only.

## Reachability and security

For security, HyperFix validates outbound HTTP destinations before saving or sending.
...
### Notifications show in HyperFix but are not delivered
