---
title: App Setup
description: Learn how to create and configure a GitHub App for HyperFix integration. Step-by-step guide to register your HyperFix instance with GitHub.
---


This guide walks you through creating and configuring a GitHub App to enable the HyperFix integration. **Start here before deploying or configuring your HyperFix instance.**

## Overview

A GitHub App provides secure, granular access to your repositories and enables powerful bidirectional synchronization between HyperFix and GitHub:

**Task to GitHub:**
- Automatically create GitHub issues when you create tasks
- Update issue status and priority labels when you change tasks
- Add comments linking to HyperFix tasks
...
- Sync GitHub labels to HyperFix task labels
...
<Warning>
**Important**: Complete this GitHub App setup **before** deploying HyperFix. You'll need the app credentials during deployment configuration.
</Warning>
...
- **GitHub App name**: `hyperfix-[your-instance-name]` (must be unique across GitHub)
- **Description**: `HyperFix project management integration`
- **Homepage URL**: Your planned HyperFix instance URL (e.g., `https://hyperfix.yourdomain.com`)
- **Webhook URL**: `https://your-hyperfix-instance.com/api/github-integration/webhook`
- **Webhook secret**: Generate a secure random string (save this for later)

<Warning>
Replace `your-hyperfix-instance.com` with your planned HyperFix domain. The webhook URL must be publicly accessible once deployed.
</Warning>
</Step>
...
<Warning>
Keep this private key secure! It's used to authenticate your HyperFix instance with GitHub.
</Warning>
</Step>
...
The GitHub App also shows an OAuth **Client ID** and **Client Secret** under "Basic information". HyperFix’s **repository integration** does not use those environment variables; they are only relevant if you enable **Sign in with GitHub** using `GITHUB_OAUTH_CLIENT_ID` / `GITHUB_OAUTH_CLIENT_SECRET` (see [GitHub SSO](/core/social-providers/github)).
...
### App Configuration
- **Use descriptive names**: Make it clear the app is for HyperFix integration
...
- Adding your organization name: `hyperfix-mycompany`
- Adding a timestamp: `hyperfix-mycompany-2024`
- Using a different naming convention: `mycompany-hyperfix`

### Webhook URL Not Accessible
Ensure your HyperFix instance is:
...
1. **[Deploy HyperFix](/core)** - Deploy your HyperFix instance with the GitHub App credentials
2. **[Configure GitHub Integration](/core/integrations/github/configuration)** - Complete the integration setup in your deployed HyperFix instance
