...
HyperFix uses S3-compatible object storage for uploads in:
...
HyperFix then serves uploaded assets back through its own API.
...
This works when HyperFix and MinIO are on the same Docker network and your browser reaches MinIO through `localhost`.
...
- HyperFix: `https://coubeche.hypeer.cloud`
- MinIO: `https://files.coubeche.hypeer.cloud`
...
- a created bucket
- MinIO CORS allowing your HyperFix origin
...
[
  {
    ...
    "AllowedOrigins": ["https://coubeche.hypeer.cloud"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
- a public bucket is not required for HyperFix's current private asset flow

## One copy-paste self-hosted example

This example gives you HyperFix + MinIO in one Compose file.
...
```env
KANEO_CLIENT_URL=https://coubeche.hypeer.cloud
KANEO_API_URL=https://coubeche.hypeer.cloud/api
...
1. Create the `hyperfix-uploads` bucket.
2. Configure MinIO CORS for your HyperFix origin.
3. Make sure `files.coubeche.hypeer.cloud` resolves publicly.
