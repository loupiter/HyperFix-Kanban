...
HyperFix uses the following services:

- `postgres`: The PostgreSQL database.
- `hyperfix`: The combined API and web container.
...
HyperFix uses S3-compatible object storage for private uploads in task descriptions and comments.

- Object storage is optional. HyperFix runs without it, but uploads in task descriptions and comments will be unavailable.
- For local/self-hosted setups, MinIO is the recommended option when you want uploads.
- The HyperFix API creates presigned upload URLs and the browser uploads files directly to the configured storage backend.
- HyperFix serves uploaded assets back through its own API, so the bucket does not need to be public.
...
If you want to use [`fs`](https://github.com/ferdzo/fs), you can point the same `S3_*` variables at its endpoint instead of MinIO. HyperFix only needs an S3-compatible API endpoint, credentials, and a bucket.
