...
For the sample environment file, see the [example .env file](https://github.com/samalehzen/hyper/blob/main/.env.sample).
...
| `DEVICE_AUTH_CLIENT_IDS` | **Optional.** Comma-separated list of allowed device authorization client IDs. When unset, HyperFix implicitly allows `hyperfix-cli` and `hyperfix-mcp` by default, so no extra configuration is required for the CLI or MCP. Override only when you need additional trusted clients, for example `hyperfix-cli,hyperfix-mcp,my-desktop-app`. |
...
HyperFix uses S3-compatible object storage for private uploads in task descriptions and task comments.
...
--- | --- | --- |
| `S3_ENDPOINT` | The S3-compatible API endpoint used by the HyperFix API for presigned uploads. Example: `http://minio:9000`. |  |
| `S3_BUCKET` | The bucket HyperFix will use for uploaded files. |  |
| `S3_ACCESS_KEY_ID` | Access key used by the HyperFix API to create presigned upload URLs. |  |
| `S3_SECRET_ACCESS_KEY` | Secret key used by the HyperFix API to create presigned upload URLs. |  |
| `S3_REGION` | The storage region used for request signing. | `us-east-1` |
| `S3_PUBLIC_BASE_URL` | Optional public base URL for uploaded assets. HyperFix does not require this for the current private asset flow. |  |
...
HyperFix supports Redis for horizontal scaling of WebSocket broadcasts. When any Redis mode is configured, multiple API instances can relay real-time updates via Redis Pub/Sub. When none are set, an in-memory adapter is used (single-instance only).
...
- If you need device authorization for clients beyond the defaults (`hyperfix-cli`, `hyperfix-mcp` when `DEVICE_AUTH_CLIENT_IDS` is unset), set `DEVICE_AUTH_CLIENT_IDS` to the full comma-separated list of trusted client IDs for your deployment.
