...
description: Configure HyperFix with your custom OpenID Connect (OIDC) provider.
---

## Overview

HyperFix supports custom OAuth 2.0 and OpenID Connect (OIDC) providers, allowing you to integrate with any standards-compliant identity provider such as Keycloak, Auth0, Okta, Azure AD, or self-hosted solutions like Pocket ID.
...
3. Set the redirect URI to: `{KANEO_API_URL}/api/auth/oauth2/callback/custom`
   - Example: `https://api.hyperfix.example.com/api/auth/oauth2/callback/custom`
...
After updating the environment variables, restart your HyperFix services:
...
```bash
CUSTOM_OAUTH_CLIENT_ID=hyperfix-client
...
For example, if `KANEO_API_URL=https://api.hyperfix.example.com`, the redirect URI should be:

```
https://api.hyperfix.example.com/api/auth/oauth2/callback/custom
```
